...
class MappingBindingValidatorSpecs extends Observes {
  ...
  val mapOp: ADifferentTestingClass => AMappedTestingClass = x => AMappedTestingClass(x.aString + "KKKKK", x.anInt + 1000, x.aFloat, x.aDecimal, x.anOptionalDouble, x.anOptionalString)
  ...
  describe("when binding and validating a testing class and all values are provided and valid") {
    ...
    it("should have returned the proper valueGetter") {
      result.instance.get should equal(AMappedTestingClass("someStringKKKKK", 1005, 88.5F, BigDecimal("900.0000009"), Some(99.87D), Some("anotherString")))
    }
  }
