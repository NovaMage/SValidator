...
object BindingFailure {
  ...
  def apply(fieldName: Option[String], error: String, cause: Option[Throwable]): BindingFailure[Nothing] = BindingFailure(List(FieldError(fieldName.getOrElse(""), MessageParts(error))), cause)
