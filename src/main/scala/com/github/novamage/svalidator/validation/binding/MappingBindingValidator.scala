...
import com.github.novamage.svalidator.validation.{Localizer, ValidationFailure}
...
abstract class MappingBindingValidator[A] extends SimpleValidator[A] {
  ...
  def bindAndValidate[B](valuesMap: Map[String, Seq[String]], mapOp: B => A)(implicit localizer: Localizer, tag: ru.TypeTag[B]): BindingAndValidationSummary[A] = {
    val bindingResult = MapToObjectBinder.bind[B](valuesMap, localizer)
    bindingResult match {
      case BindingFailure(errors, _) => Failure(errors.map(error => ValidationFailure(error.fieldName, error.errorMessage, Map.empty)), valuesMap, None)
      case BindingPass(value) =>
        val mappedValue = mapOp(value)
        val validatedValue = validate(mappedValue, implicitly)
