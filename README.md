...
The `be` and `have` constructs provide a series of convenient methods according to the type of type of the field being validated.  For example, for fields of type String(such as firstName) the empty() view method is provided.  But for Int fields such as `age`, a method `negative()` is provided.  Additionaly, by importing the
...
By using the `when` clause, you can add conditional stamenents to fields only udner specific circumstances, as shown above.
